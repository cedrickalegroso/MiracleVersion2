if not ConfigDP.GUI.Enabled then return end
...
function SoundPlay(which)
	if not ConfigDP.GUI.Sound then return end
...
local function DrawButton(b)
	local B = ConfigDP.GUI.ButtonColor
	...
	elseif IsDisabledControlJustPressed(1, 25) and ConfigDP.Debug then
		local x,y = GetCursor()
		local Distance = Distance(b.x+0.005, b.y+0.025, x, y)
...
local function Check(ped) -- We check if the player should be able to open the menu.
	if IsPedInAnyVehicle(ped) and not ConfigDP.GUI.AllowInCars then
		return false
	elseif IsPedSwimmingUnderWater(ped) then
		return false
	elseif IsPedRagdoll(ped) and not ConfigDP.GUI.AllowWhenRagdolled then
		return false
...
local function GenerateTheButtons() -- We generate the buttons here to save on a little bit of performance.
	local x, y, rx, ry = ConfigDP.GUI.Position.x, ConfigDP.GUI.Position.y, 0.1, 0.175
	for k,v in pairs(ConfigDP.Commands) do
		local i = v.Button
	...
	if ConfigDP.ExtrasEnabled then -- The extra buttons arent tied to the wheel, and can be moved with simple offsets.
		for k,v in pairs(ConfigDP.ExtraCommands) do
...
local function HoveredButton()
	...
	for k,v in pairs(Buttons) do
		...
		if Distance < 0.025 then
			Text(ConfigDP.GUI.Position.x, ConfigDP.GUI.Position.y-0.10, 0.3, v.Text, false, false, true)
			Text(ConfigDP.GUI.Position.x, ConfigDP.GUI.Position.y-0.08, 0.22, v.Desc, {210,210,210}, false, true, {x = 0.1, y = 0.2})
		end
	end
	for k,v in pairs(ExtraButtons) do
		if v.Enabled then
			local Distance = Distance(ConfigDP.GUI.Position.x+v.OffsetX+0.005, ConfigDP.GUI.Position.y+v.OffsetY+0.025, x, y)
	...
	local Distance = Distance(ConfigDP.GUI.Position.x+0.005, ConfigDP.GUI.Position.y+0.025, x, y)
	if Distance < 0.015 then
		Text(ConfigDP.GUI.Position.x, ConfigDP.GUI.Position.y-0.09, 0.3, Lang("Info"), false, false, true)
	end
end
...
Citizen.CreateThread(function()
	...
	while true do Wait(0)
		if not ConfigDP.GUI.Toggle then
			if IsControlPressed(1, ConfigDP.GUI.Key) then
				local Ped = PlayerPedId() 
				if Check(Ped) then MenuOpened = true end
			else MenuOpened = false end
			if IsControlJustPressed(1, ConfigDP.GUI.Key) then
				local Ped = PlayerPedId() 
				if Check(Ped) then SoundPlay("Open") SetCursorLocation(ConfigDP.GUI.Position.x, ConfigDP.GUI.Position.y) end
			elseif IsControlJustReleased(1, ConfigDP.GUI.Key) then
				if Check(Ped) then MenuOpened = false SoundPlay("Close") end
			end
		else
			if IsControlJustPressed(1, ConfigDP.GUI.Key) then
				local Ped = PlayerPedId() 
				if Check(Ped) then SoundPlay("Open") SetCursorLocation(ConfigDP.GUI.Position.x, ConfigDP.GUI.Position.y) MenuOpened = not MenuOpened end
			end
		end
		if MenuOpened then DrawGUI() end
		if ConfigDP.Debug then DrawDev() end
	end
end)