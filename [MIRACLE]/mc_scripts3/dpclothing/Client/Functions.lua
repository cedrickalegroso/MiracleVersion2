...
function log(l) -- Just a simple logging thing, to easily log all kinds of stuff.
	...
	if not ConfigDP.Debug then return end
...
function Text(x, y, scale, text, colour, align, force, w)
	...
	local colour = colour or ConfigDP.GUI.TextColor
	SetTextFont(ConfigDP.GUI.TextFont)
	SetTextJustification(align)
	SetTextScale(scale, scale)
	SetTextColour(colour[1], colour[2], colour[3], 255)
	if ConfigDP.GUI.TextOutline then SetTextOutline() end	
...
function Lang(what)
	local Dict = Locale[ConfigDP.Language]
...
AddEventHandler('dpc:ToggleMenu', function()
	...
	if MenuOpened then SoundPlay("Open") SetCursorLocation(ConfigDP.GUI.Position.x, ConfigDP.GUI.Position.y) else SoundPlay("Close") end
end)
