...
RegisterServerEvent('ax1s_crafting:getPlayerInventory')
AddEventHandler('ax1s_crafting:getPlayerInventory', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer ~= nil then
		TriggerClientEvent('ax1s_crafting:openMenu', _source, xPlayer.inventory)
	end
end)
...
RegisterServerEvent('ax1s_crafting:craftItem')
AddEventHandler('ax1s_crafting:craftItem', function(ingredients)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local item = findRecipe(ingredients)
	if not item then
		TriggerClientEvent('esx:showNotification', _source, 'No recipes found with these ingredients')
	else
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~You Crafted: ~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, 'You don\'t have all the ingredients')
			end
		end
	end
end)
