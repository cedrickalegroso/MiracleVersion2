...
Citizen.CreateThread(function()
	for k, v in pairs(ConfigHoldaper.Stores) do
...
AddEventHandler('esx_holdup:starttimer', function()
	timer = ConfigHoldaper.Stores[store].secondsRemaining
...
Citizen.CreateThread(function()
	while true do
		...
		if holdingUp then
			if GetDistanceBetweenCoords(plyCoords, ConfigHoldaper.Stores[store].coords) > 15.0 then
