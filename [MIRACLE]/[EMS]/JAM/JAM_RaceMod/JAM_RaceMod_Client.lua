			...
			if JUtils:GetVecDist(plyPos, self.RaceJoinPos) > self.LeaveDist then
				...
				Citizen.CreateThread(function()
					...
					while tick < 10 do 
						...
						TriggerEvent('JAM_Notify:ShowNotification', "~r~Du hast das Rennen verlassen.")
					end
				end)
			elseif JUtils:GetVecDist(plyPos, self.RaceJoinPos) > self.LeaveWarnDist then
				TriggerEvent('JAM_Notify:ShowNotification', "~r~Entferne dich nicht zuweit vom Start!")
			end
		end
	end
end

RegisterCommand('showJob2', function(source, args)
	...
	if plyData and plyData.job and plyData.job.label and plyData.job.grade_label then
		TriggerEvent('JAM_Notify:ShowNotification', "Job : [~g~"..plyData.job.label.."~s~] : Rang : [~g~"..plyData.job.grade_label.."~s~]")
	else 
		TriggerEvent('JAM_Notify:ShowNotification', "Jobdaten nicht gefunden.")
	end
end)
...
function JRM:SetupRace(wager)
	...
	if not IsPedInAnyVehicle(plyPed) then
		TriggerEvent('JAM_Notify:ShowNotification', "~r~Du musst in einem Fahrzeug sein.")
		return
	end
	...
	if DoesBlipExist(blip) then
		...
		TriggerEvent('JAM_Notify:ShowNotification', "Du musst zuerst einen ~r~Wegpunkt ~s~setzen.")
		return
	end	
	...
	if wager > 0 then
		...
		if plyData.money >= wager then 
			...
			TriggerEvent('JAM_Notify:ShowNotification', "Du hast ~r~nicht ~s~genug ~r~Geld~s~.")
			return
		end
	end
	...
	TriggerEvent('JAM_Notifications:Notify', "Strassenrennen im Gange.", 0, blipCoord, "General", "police")

	Citizen.CreateThread(function()	
		local timer = GetGameTimer()
		while (GetGameTimer() - timer) < (self.WaitForPlayersTimer * 1000) and JRM.RaceJoinPos do	
			Citizen.Wait(0)	
			TriggerEvent('JAM_Notify:ShowNotification', "Du hast ein ~g~Strassenrennen ~g~gestartet~s~. Vordere Spieler in der nähe heraus.")
		end
	end)
	...
	Citizen.CreateThread(function()
		ESX.TriggerServerCallback('JAM_RaceMod:SetupRace', function(playersJoined)
			...
			if playersJoined > 1 then
				...
				TriggerEvent('JAM_Notify:ShowNotification', "~r~Niemand ~s~ist dem Rennen beigetreten.")
				TriggerServerEvent('JAM_RaceMod:SetMoney', wager)
				JRM.RaceID = false
				JRM.RaceWager = false
...
AddEventHandler('JAM_RaceMod:ChallengeNearbyPlayers', function(racePos, raceID, wager)
	...
	if dist < JRM.JoinDistLimit then
		Citizen.CreateThread(function(...) 
			...
			local tick = 0
			local str = ""
			if wager and wager > 0 then str = "Richte dein Fahrzeug aus und drücke [~g~E~s~] um dem Rennen beizutreten. Wette: $~g~" .. wager
			else str = "Richte dein Fahrzeug aus und drücke [~g~E~s~] um dem Rennen beizutreten."; end
			...
			if not JRM.RaceID then 
				if wager > 0 and not canJoin then
					TriggerEvent('JAM_Notify:ShowNotification', "Du hast ~r~nicht ~s~genug ~r~Geld ~s~um dem Rennen beizutreten.")
				else
					TriggerEvent('JAM_Notify:ShowNotification', "Du bist dem Rennen ~r~nihct ~s~beigetreten.")
				end
			else 
				TriggerEvent('JAM_Notify:ShowNotification', "Du bist dem Rennen ~g~beigetreten ~s~.")
			end
		end)
	end
end)
...
AddEventHandler('JAM_RaceMod:BeginRace', function(raceID, blipCoord)
	...
	Citizen.CreateThread(function() 		
		...
		while (GetGameTimer() - timer) < ((JRM.StartTimer - 1) * 1000) and JRM.RaceJoinPos do
			...
			local str = "~r~"..counter
			TriggerEvent('JAM_Notify:ShowNotification', "Du wirst auf der Position gefreezed in "..str.." Sekunden~s~. Auf die Startpostion.")
			Citizen.Wait(10)
		end
		...
		Citizen.Wait(0)
		TriggerEvent('JAM_Notify:ShowNotification', "~g~LOS!", 1)
...
function JRM:FinishRace()
	...
	ESX.TriggerServerCallback('JAM_RaceMod:FinishStreetRace', function(position, wager, players)
		local str = "Du beendest das Rennen auf folgender Position : "
		if position == 1 then
			if wager > 0 then
				local plyData = ESX.GetPlayerData()
				local prize = wager * players
				TriggerServerEvent('JAM_RaceMod:SetMoney', prize)
				str = str .. "[~g~" ..position.. "~s~] : Du gewinnst : $~g~" ..prize
...
AddEventHandler('JAM_RaceMod:Timeout', function() 
	if not JRM.RaceID then
		TriggerEvent('JAM_Notify:ShowNotification', "~r~Du hast das Rennen nicht beendet.")
		JRM.RaceID = false
		JRM.RaceWager = false
		JRM.RaceJoinPos = false
