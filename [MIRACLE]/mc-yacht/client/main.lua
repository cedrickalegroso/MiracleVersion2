                ...
                if GetDistanceBetweenCoords(GetEntityCoords(ped), -2084.88, -1018.12, 12.78, true) < 1 then
                    ESX.ShowHelpNotification(_U('start_robbery'))
                        if IsControlJustReleased(1, 51) then
                            TriggerServerEvent("mc-yacht:robbery")
							--TriggerServerEvent('MF_Trackables:Notify','Robbery in Yacht.',GetEntityCoords(GetPlayerPed(-1)),'police')
							--TriggerServerEvent('MF_Trackables:Notify',"Robbery in Yacht.",GetEntityCoords(GetPlayerPed(-1)),"ambulance")
                        end
                    end
                end
            end
...
RegisterNetEvent("mc-yacht:start")
AddEventHandler("mc-yacht:start", function()
  heist = true
  tags = true
  Citizen.CreateThread(function()
    while true do 
...
Citizen.CreateThread(function()
    while searched < 20 do 
        Citizen.Wait(100)
            if searched == 20 then
                TriggerServerEvent("mc-yacht:reward")
                heist = false
                tags = false
                Citizen.Wait(19000)
...
RegisterNetEvent("mc-yacht:notification")
AddEventHandler("mc-yacht:notification", function(text)
    ESX.ShowHelpNotification(text)
end)

RegisterNetEvent("mc-yacht:notify")
AddEventHandler("mc-yacht:notify", function(text)
