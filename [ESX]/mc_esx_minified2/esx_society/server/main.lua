...
AddEventHandler('esx_society:withdrawMoney', function(society, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'withdrawn '.. ESX.Math.GroupDigits(amount)})
...
AddEventHandler('esx_society:depositMoney2', function(society, amount)
	...
	amount = ESX.Math.Round(tonumber(amount))
		...
		if amount > 0 and xPlayer.getMoney() >= amount then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'deposited '.. ESX.Math.GroupDigits(amount)})
		...
		MySQL.Async.fetchAll('SELECT firstname, lastname, identifier, job2, job2_grade FROM users WHERE job2 = @job2 ORDER BY job_grade DESC', {
			...
			for i=1, #results, 1 do
				table.insert(employees, {
					...
					--	grade_name  = Jobs[results[i].job2].grades[tostring(results[i].job_grade)].name,
					--	grade_label = Jobs[results[i].job2].grades[tostring(results[i].job_grade)].label
					}
				})
			end
